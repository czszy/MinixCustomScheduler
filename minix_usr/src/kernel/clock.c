...
#define GET_PROC_TIME(p) SCHED_RATE

#define GET_TICKS(p) sched_ticks

#else

#define GET_GROUP_TIME(group) (group_times[group]*HZ/1000)
/*#define GET_GROUP_TIME(group) (120*HZ/1000)*/
#define GET_PROC_TIME(p) (p->p_priority == PPRI_USER ? \
			GET_GROUP_TIME(p->group) : SCHED_RATE)

#define GET_TICKS(p) sched_ticks[p->group]
...
#else
PRIVATE int sched_ticks[PROC_GROUP_MAX] = {SCHED_RATE,SCHED_RATE,SCHED_RATE};
#endif
 ...
 */
  ...
  if (--GET_TICKS(bill_ptr) == 0)
	{
		if (bill_ptr == prev_ptr)
			lock_sched();	/* process has run too long */
		
		/* reset quantum */
		GET_TICKS(bill_ptr) = GET_PROC_TIME(bill_ptr);

		/* new previous process */
		prev_ptr = bill_ptr;
  }
}
 ...
 */
  ...
  if (GET_TICKS(bill_ptr) == 1) fd_timer();		/* floppy deselect */
#endif

  if (next_alarm <= now ||
      GET_TICKS(bill_ptr) == 1 &&
      bill_ptr == prev_ptr &&
  ...
  if (--GET_TICKS(bill_ptr) == 0) {
	...
	GET_TICKS(bill_ptr) = GET_PROC_TIME(bill_ptr);/* reset quantum */
