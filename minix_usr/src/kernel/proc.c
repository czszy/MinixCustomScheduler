...
void linkGroup(struct proc *rp);
void unlinkGroup(struct proc *rp);
void moveProcInGroup(int group);
...
PUBLIC void set_proc_group(rp, group)
struct proc *rp;
int group;
{
	unlinkGroup(rp);
...
struct proc* findFirstInGroup(int group)
{
	struct proc* tmp;
	tmp = rdy_head[USER_Q];
	while(tmp != NIL_PROC)
  ...
  if ( (rp = rdy_head[USER_Q]) != NIL_PROC)
	{
		...
		/*rp = findFirstInGroup(1);
		if(rp)
		{
			rp = rdy_head[USER_Q];
			printf("g: %c, pick: %d, %d, %d\n", 'a'+curr_group, rp->p_pid, rp->p_nextready ? 
					rp->p_nextready->p_pid : 0,
					rp->p_nextready ?
						rp->p_nextready->p_nextready ?
						rp->p_nextready->p_nextready->p_pid :
						0 :
						0);
		}*/
		...
		/*rp = findFirstInGroup(curr_group);*/
		/*rp = findFirstInGroup(0);*/
		
		/*if(rp == NULL)
		{
			printf("Missing proc in group %c!\n", curr_group + 'a');
			rp = rdy_head[USER_Q];
			curr_group = rp->group;
		}*/
		
		bill_ptr = proc_ptr = rp;
		return;
  }
 ...
 */
	register int group, prevGroup;
	register struct proc* rp1;
	register struct proc* rp2;
	register struct proc* head;

  if ((head = rdy_head[USER_Q]) == NIL_PROC)
		return;
	
	prevGroup = curr_group;
	
	/* Check if head process has time left */
	if(head->timeUsed < group_times[head->group] - 1)
	{
		/* Add single tick */
		head->timeUsed++;

		/*
		printf("Tick: %d group: %c, used: %d\n", head->p_pid, head->group + 'a', head->timeUsed);
		*/
	...
	else
	{
		...
		do
		{
			group = (group + 1) % PROC_GROUP_MAX;

			/* find next process in that group and cache element before it */
			/*rp1 = findFirstInGroup(group);*/
			rp2 = head;
			rp1 = rp2->p_nextready;
			while(rp1)
			{
		...
		if(rp1 == NULL)
		{
			/*printf("Missing process after %d (%c)\n", head->p_pid, 'a' + head->group);*/
		}
		else
		{
			...
			rp2->p_nextready = rp1->p_nextready;
			rp1->p_nextready = head->p_nextready;
			rdy_head[USER_Q] = rp1;
	...
	/* Debug group changes */
	/*if(curr_group != prevGroup)
		printf("Zmiana grupy: %c -> %c", 'a' + prevGroup, 'a' + curr_group);*/
	/*else
		printf("Grupa: %c", 'a' + curr_group);*/

	/* Update current process if need to */
	if(head->timeUsed <= 0)
  	pick_proc();
}
 ...
 */
  ...
  if (isservp(rp)) {		/* others are similar */
	...
	/* link user process to the group */
	linkGroup(rp);
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
	...
	/* unlink user process from the group */
	unlinkGroup(rp);
