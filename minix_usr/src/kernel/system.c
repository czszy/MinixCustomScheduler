...
FORWARD _PROTOTYPE( int do_setgrouptime, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_SETGROUPTIME: r = do_setgrouptime(&m);break;
...
/* 
  Change process group 
	...
	Result:
	1:      Group was the same
  2:      Group was changed
	EINVAL: Invalid pid or group
...
message *m_ptr;			/* pointer to request message */
{
  struct proc *rp;
	...
	/* check group */
	if(m_ptr->m1_i2 < PROC_GROUP_A || m_ptr->m1_i2 >= PROC_GROUP_MAX)	
		return EINVAL;
  ...
  if(rp != NIL_PROC)
  {
    /* check if group won't change */
    if(rp->group == m_ptr->m1_i2)
    {
			return 1;
    }

    /* change process group */
		set_proc_group(rp, m_ptr->m1_i2);
...
/* 
  Change group time
	...
	1:      Time was the same
  2:      Time was changed
	EINVAL: Invalid time or group

*/
PRIVATE int do_setgrouptime(m_ptr)
message *m_ptr;			/* pointer to request message */
{
	/* check time and group */
	if(m_ptr->m1_i1 < 100 || m_ptr->m1_i1 > 1000 ||
		m_ptr->m1_i2 < PROC_GROUP_A || m_ptr->m1_i2 >= PROC_GROUP_MAX)
		return EINVAL;

	/* check if group time won't change */
	if(group_times[m_ptr->m1_i2] != m_ptr->m1_i1)
		return 1;

	/* change group time */
	group_times[m_ptr->m1_i2] = m_ptr->m1_i1 / 100;/* z clock.c czas */
